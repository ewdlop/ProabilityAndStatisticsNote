674b227d-11c8-8012-ac6d-bb04699bc30f
...
- Four of a Kind:
\[
C(13, 1) \times C(12, 1) \times C(4, 4) \times C(48, 1) = 624
\]
...
1. **Choose the Rank for the Four of a Kind: \( C(13, 1) \)**
   - There are 13 possible ranks (Ace, 2, 3, ..., King).
   - We need to choose **one** rank for our Four of a Kind.
   - \( C(13, 1) = 13 \).

2. **Choose the Rank for the Additional Card: \( C(12, 1) \)**
   - After choosing the rank for the Four of a Kind, 12 ranks remain for the fifth card.
   - We need to choose **one** of these remaining ranks.
   - \( C(12, 1) = 12 \).

3. **Choose All Four Suits for the Four of a Kind: \( C(4, 4) \)**
   - For the Four of a Kind, we need all four suits (e.g., \( \heartsuit, \clubsuit, \spadesuit, \diamondsuit \)).
   - There's only **one way** to choose all four suits.
   - \( C(4, 4) = 1 \).

4. **Choose One Card from the Remaining Deck: \( C(48, 1) \)**
...
Now multiply the combinations:
\[
C(13, 1) \times C(12, 1) \times C(4, 4) \times C(48, 1) = 13 \times 12 \times 1 \times 48 = 624
\]

### **Why Does This Work?**
This formula counts all possible ways to form a valid Four of a Kind hand:
- First, we pick the rank for the Four of a Kind (\( 13 \) choices).
- Then, we pick the rank for the kicker (the extra card, \( 12 \) choices).
- Next, we pick all four suits for the Four of a Kind (\( 1 \) way).
- Finally, we pick one card from the remaining deck to complete the hand (\( 48 \) ways).

This ensures that all valid combinations are considered without overcounting or missing any possibilities.
